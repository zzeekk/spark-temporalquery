...
object TemporalQueryUtil extends Serializable with Logging {
  ...
  implicit class TemporalDataFrameExtensions(df1: DataFrame) {
     ...
     */
    def temporalFullJoin(df2: DataFrame, keys: Seq[String], rnkExpressions: Seq[Column] = Seq(),
                         additionalJoinFilterCondition: Column = lit(true), doCleanupExtend: Boolean = true)
                        (implicit ss: SparkSession, tc: TemporalQueryConfig): DataFrame = IntervalQueryImpl
     ...
     */
    def temporalLeftJoin(df2: DataFrame, keys: Seq[String], rnkExpressions: Seq[Column] = Seq(),
                         additionalJoinFilterCondition: Column = lit(true), doCleanupExtend: Boolean = true)
                        (implicit ss: SparkSession, tc: TemporalQueryConfig): DataFrame = IntervalQueryImpl
     ...
     */
    def temporalRightJoin(df2: DataFrame, keys: Seq[String], rnkExpressions: Seq[Column] = Seq(),
                          additionalJoinFilterCondition: Column = lit(true), doCleanupExtend: Boolean = true)
                         (implicit ss: SparkSession, tc: TemporalQueryConfig): DataFrame = IntervalQueryImpl
     ...
     */
    def temporalCombine(ignoreColNames: Seq[String] = Seq())
                       (implicit ss: SparkSession, tc: TemporalQueryConfig): DataFrame = IntervalQueryImpl
     ...
     */
    def temporalExtendRange(keys: Seq[String] = Seq(), extendMin: Boolean = true, extendMax: Boolean = true)
                           (implicit ss: SparkSession, tc: TemporalQueryConfig): DataFrame = IntervalQueryImpl
    ...
    /**
     * Transforms [[DataFrame]] with continuous time,
     * half open time intervals [fromColName , toColName [, to discrete time ([fromColName , toColName])
     *
     * Note: This function needs TemporalQueryConfig with a ClosedInterval definition
     *
