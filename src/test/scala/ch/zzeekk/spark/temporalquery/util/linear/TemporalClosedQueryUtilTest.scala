...
class TemporalClosedQueryUtilTest extends AnyFlatSpec with Matchers with TestUtils {
  ...
  "rangeFullJoin_rightMapWithGapsAndRnkExpressions" should "return expected results" in {
    ...
    val actual =
      dfLeft.rangeFullJoin(df2 = argumentRight, keys = Seq("id"), rnkExpressions = Seq($"img", defaultTemporalConfig.fromCol))
    val expected = Seq(
      // img = {}
      (0, Some(4.2), None, "2017-12-10 00:00:00", "2017-12-31 23:59:59.999"),
      // img = {A}
  ...
  "rangeLeftJoin dfLeft with dfRight" should "return expected results" in {
    ...
    val expected = Seq(
      ...
      .toDF("id", "value_l", "value_r", defaultTemporalConfig.fromColName, defaultTemporalConfig.toColName)
    val result = dfEqual(actual, expected)
    if (!result) printFailedTestResult("rangeLeftJoin_dfLeft_dfRight", Seq(dfLeft, dfRight))(actual, expected)
    result shouldBe true
  }
  ...
  "rangeLeftJoin dfLeft with dfEmpty" should "add an empty column value_r to dfLeft" in {
    ...
    val result = dfEqual(actual, expected)
    if (!result) printFailedTestResult("rangeLeftJoin_dfLeft_dfEmpty", Seq(dfLeft, dfEmpty))(actual, expected)
    result shouldBe true
  }
  ...
  "rangeLeftJoin_rightMapWithGapsAndRnkExpressions" should "return expected results" in {
    ...
    val argumentRight = Seq(
      ...
      .toDF("id", defaultTemporalConfig.fromColName, defaultTemporalConfig.toColName, "img")
    val actual =
      dfLeft.rangeLeftJoin(df2 = argumentRight, keys = Seq("id"), rnkExpressions = Seq($"img", defaultTemporalConfig.fromCol))
        .rangeCombine()
    val expected = Seq(
      // img = {}
      (0, 4.2, None, "2017-12-10 00:00:00", "2017-12-31 23:59:59.999"),
  ...
  "rangeLeftJoin with equally named columns apart join columns" should "return expected results" in {
    val dfL = dfLeft.withColumnRenamed("value_l", "value").as("dfL")
    val dfR = dfRight.withColumnRenamed("value_r", "value").as("dfR")
    val actual = dfL.rangeLeftJoin(dfR, Seq("id")).orderBy($"id", defaultTemporalConfig.fromCol)
  ...
  "rangeRightJoin_rightMapWithGapsAndRnkExpressions" should "return expected results" in {
    ...
    val actual =
      dfLeft.rangeRightJoin(df2 = argumentRight, keys = Seq("id"), rnkExpressions = Seq($"img", defaultTemporalConfig.fromCol))
        .rangeCombine()
    val expected = Seq(
      // img = {}
      (0, Some(4.2), Some("A"), "2018-01-01 00:00:00",     "2018-01-31 23:59:59.999"),
